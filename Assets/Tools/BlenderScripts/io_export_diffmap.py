...
def post(ob):
    ...
    for n in range(len(original_face_mat_indices)-1):
        mesh.faces[n].material_index = original_face_mat_indices[n]
...
def generate_diffmap_from_shape(ob, filepath, name, shape, shapeson, width=128, height=128, margin=10 ):
    ...
    if shapeson == True:    
        ...
        elif bpy.app.build_platform.find("Linux") != -1:       
            path = filepath + '/' + name + '-' + shape.name + '.tga'
...
class EXPORT_OT_tools_diffmap_exporter(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        #if len(name[0]) > 0:
        #    scene.DiffMapSettings.name = name[0]
...
def menu_func(self, context):
    ...
    self.layout.operator(EXPORT_OT_tools_diffmap_exporter.bl_idname, text="Export MetaMorph ShapeKeys to DiffMaps and Animations").filepath = default_path
